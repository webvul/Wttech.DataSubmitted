...
$(document).ready(function () {
    ...
    $("#export").click(function () {
        ...
        $.ajax({
            ...
            data: { "StartTime": StartTime, "ReportType": 4, "HolidayId": 8 },
            dataType: "json",
            type: "Post",
            beforeSend: function () {
                isShowload(1);
            ...
            success: function (data) {
                ...
                window.location = "/DownFile/Downfile?name=" + path + "&reporttype=4";
            }
        });
    });
    //修改
    $("#upd").click(function () {
        ...
        var flag = 0;
        if (type == 1) {
            var ids = $("#report4").jqGrid("getDataIDs");
            $.each(ids, function (a, b) {
                var rowData = $("#report4").jqGrid('getRowData', b);
                //if (rowData.VehNum != "0" && b != "NaN") {
                if (b != "NaN") {
                    flag = 1;
                    jQuery("#report4").jqGrid('editRow', b);
                }
            })
            if (flag == 1) {
                $("#upd").hide();
    ...
    $("#saveBtn").click(function () {
        ...
        var i = 0;
        $.each(ids, function (a, b) {
            rowData = $("#report4").jqGrid('getRowData', b);
            if (rowData.VehNum != "0") {
                dataP[i++] = {
                    "ExEn": rowData.ExEn,
        ...
        var ReportData = { "DataDate": DataDate, "DataInfo": dataP, "ReportType": 4 };
        $.ajax({
            dataType: "json",
            type: "POST",
            url: "/TJReport/UpdateImpStaDaily",
            data: ReportData,
            success: function (data) {
                if (data.ResultKey == 1) {
    ...
    $("#cancleBtn").click(function () {
        ...
        if (confirm("是否取消对当前报表的修改？")) {
            ...
            $.each(ids, function (a, b) {
                var rowData = $("#report4").jqGrid('getRowData', b);
                jQuery("#report4").jqGrid('restoreRow', b);
            })
            $("#upd").show();
            $("#saveBtn").hide();
            $("#cancleBtn").hide();
...
function getReport() {
    ...
    $.ajax({
        url: '/Report/Getinfo',
        data: { "StartTime": StartTime, "ReportType": 4 },
        dataType: "json",
        type: "Post",
        beforeSend: function () {
            isShowload(1);
        ...
        success: function (data) {
            ...
            grid4.data = data.ReportData;
            $("#report4").jqGrid(grid4).trigger("reloadGrid");
            $("#report4").jqGrid('setGridWidth', $("#maincontent").width() - 30);
   //         $("#report4").jqGrid('setGridHeight', $(window).height() - 575);
            $("#report4").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'hidden' });
        }
    });
            ...
            buttons: {
                "确定": function () {
                    ...
                    $.ajax({
                        ...
                        data: { "StartTime": StartTime, "ReportType": 4, "FloatingRange": FloatingRange, "HolidayId": 8 },
                        success: function (data) {
                            if (data.ResultKey == 0) {
                                alert(data.ResultValue);
                            } else if (data.ResultKey == 1) {
