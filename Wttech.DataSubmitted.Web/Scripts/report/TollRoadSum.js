...
$(document).ready(function () {
    ...
    $("#export").click(function () {
        ...
        $.ajax({
            ...
            data: { "StartTime": StartTime, "ReportType": 1, "HolidayId": 8 },
            dataType: "json",
            type: "Post",
            beforeSend: function () {
                isShowload(1);
            ...
            success: function (data) {
                ...
                window.location = "/DownFile/Downfile?name=" + path + "&reporttype=1";
            }
        });
    });
    //修改
    $("#upd").click(function () {
        ...
        var flag = 0;
        if (type == 1) {
            var ids = $("#report1").jqGrid("getDataIDs");
            $.each(ids, function (a, b) {
                var rowData = $("#report1").jqGrid('getRowData', b);
                //if (rowData.VehNum != "0" && b != "NaN") {
                if ( b != "NaN") {
                    flag = 1;
                    jQuery("#report1").jqGrid('editRow', b);
                }
            })
            if (flag == 1) {
                $("#upd").hide();
    ...
    $("#saveBtn").click(function () {
        ...
        var i = 0;
        $.each(ids, function (a, b) {
            rowData = $("#report1").jqGrid('getRowData', b);
            if (rowData.VehNum != "0") {
                dataP[i++] = {
                    "ExEn": rowData.ExEn,
        ...
        var ReportData = { "DataDate": DataDate, "DataInfo": dataP, "ReportType": 1 };
        $.ajax({
            dataType: "json",
            type: "POST",
            url: "/BeijingReport/UpdateTollRoadSum",
            data: ReportData,
            success: function (data) {
                if (data.ResultKey == 1) {
    ...
    $("#cancleBtn").click(function () {
        ...
        if (confirm("是否取消对当前报表的修改？")) {
            ...
            $.each(ids, function (a, b) {
                var rowData = $("#report1").jqGrid('getRowData', b);
                jQuery("#report1").jqGrid('restoreRow', b);
            })
            $("#upd").show();
            $("#saveBtn").hide();
            $("#cancleBtn").hide();
...
function getReport() {
    ...
    $.ajax({
        url: '/Report/Getinfo',
        data: { "StartTime": StartTime, "ReportType": 1 },
        dataType: "json",
        type: "Post",
        beforeSend: function () {
            isShowload(1);
        ...
        success: function (data) {
            ...
            $("#report1").jqGrid(reportgrid).trigger("reloadGrid");
            $("#report1").jqGrid('setGridWidth', $("#maincontent").width() - 30);
            //$("#report1").jqGrid('setGridHeight', $(window).height() - 575);
            $("#report1").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'hidden' });
            $("#report1").closest(".ui-jqgrid-bdiv").css({ 'overflow-y': 'hidden' });
            changeHeader();
            //jQuery("#report1").jqGrid("setComplexHeaders", complexoption);
        }
    });
...
function changeHeader() {
    if (!isHb) {
        $("#report1").jqGrid('setGroupHeaders', {
            ...
            buttons: {
                "确定": function () {
                    ...
                    $.ajax({
                        ...
                        data: { "StartTime": StartTime, "ReportType": 1, "FloatingRange": FloatingRange ,"HolidayId": 8},
                        success: function (data) {
                            if (data.ResultKey == 0) {
                                alert(data.ResultValue);
                            } else if (data.ResultKey == 1) {
                    ...
                    $(".forecastIcon+span").css({ "color": "#555" });
                },
                "取消": function () {
                    $(this).dialog("close");
                    $(".forecastIcon").removeClass("forecastIcon_select");
